...
const checkMultiIpDetection = require("../utils/multiIpDetection");
...
/* ===============================
   🔐 LOGIN (PASSWORD)
   =============================== */
router.post("/login", ipFilter, async (req, res) => {
  try {
    ...
    // 🔍 Multi-IP Detection — runs for every login
    const ipAddress =
      req.headers["x-forwarded-for"] || req.socket.remoteAddress || "unknown";
    checkMultiIpDetection(admin._id.toString(), ipAddress, req.io);

    // ✈️ Impossible Travel Detection
    const latitude = req.body.latitude || null;
    const longitude = req.body.longitude || null;
    if (latitude && longitude) {
      checkImpossibleTravel(admin._id.toString(), ipAddress, latitude, longitude, req.io);
    }
...
/* ===============================
   📲 MFA SETUP
   =============================== */
router.post("/mfa/setup", adminMfaAuth, async (req, res) => {
  try {
